...
function get_f(eom, k::Vector, model::Symbol, dn, single=false)
    ...
    for j in 1:num_j
        ...
        Threads.@threads for i in 1:N
            Y[i] = π/4 / (m_tilde[i]*j)^3 * ρ_new[indices[i]]^2 * abs2(c_n[i, j]) / H_new[indices[i]]
        end
        
        # to be interpolated as k/a_e H_e
        X = @. a_new[indices[1:end-1]] * m_tilde * j / aH_e
        # @info "Production of first inflaton oscillation at k/a_e H_e = " X[1]
        # @show X

        # correction factor 
        C = @. 1/abs(1 + 1/j * X * aH_e / (a_new * H_new)[indices[1:end-1]] * mdm2)
